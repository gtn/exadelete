...
function block_exadelete_clean_anonimize_users(array $users) {
    global $DB;
    if (!$users) {
        return true;
    }
    ...
    $delete_these_tables = array("assignfeedback_editpdf_quick", "assignment_submissions", "assign_grades", "assign_submission",
    ...
    foreach ($users as $user) {
        ...
        foreach ($delete_these_tables as $table) {
            try {
                ...
                // table not found or something else with DB
            }
        }
        ...
        $return['message'] = \block_exadelete\get_string('alluserdatadeleted', null)." ".fullname($user)."<br/>";
    }
